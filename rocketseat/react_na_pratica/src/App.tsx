...
export function App() {
  ...
  const maxPerPage = searchParams.get('_per_page') ? Number(searchParams.get('_per_page')) : 10

  const { data: tagsResponse, isLoading, isFetching } = useQuery<TagResponse>({
    queryKey: ['get-tags', urlFilter, page, maxPerPage],
    queryFn: async () => {
      const response = await fetch(`http://localhost:3333/tags?_page=${page}&_per_page=${maxPerPage}&title=${urlFilter}`)
