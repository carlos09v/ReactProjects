...
export function Pagination({ items, page, pages }: PaginationProps) {
  ...
  const handleMaxPerPage = (value: string) => {
    setSearchParams(params => {
  ...
  return (
    <div className="flex text-sm items-center justify-between text-zinc-500">
      <span>Showing {searchParams.get('_per_page') ?? 10} of {items} items</span>
      <div className="flex items-center gap-8">
        <div className="flex items-center gap-2">
          <span>Rows per page</span>

          <Select onValueChange={handleMaxPerPage}  defaultValue={searchParams.get('_per_page') ?? "10"}>
